...
func main() {
	...
	// Extract only -w, -l, -W, -L, -sw, -sl, -v for this process. For child, restore ALL original flags (replace ONLY -p value!!!)
	var wIP, lIP string
	var swIP, slIP string // Source IPs for forwarding
	var WPort, LPort int
	...
	for i := 0; i < len(args); i++ {
		...
		if arg == "-sw" && i+1 < len(args) {
			swIP = args[i+1]
		}
		if arg == "-sl" && i+1 < len(args) {
			slIP = args[i+1]
		}
		if arg == "-W" && i+1 < len(args) {
	...
	if WPort > 0 && wIP != "" {
		// If source IP (-sw) is specified, -w becomes listenIP and -sw becomes targetIP
		// Otherwise, both listenIP and targetIP are -w (same IP)
		listenIP := wIP
		targetIP := wIP
		if swIP != "" {
			targetIP = swIP
		}
		pairs = append(pairs, forwardPair{listenIP: listenIP, targetIP: targetIP, targetPort: WPort})
	}
	if LPort > 0 && lIP != "" {
		// If source IP (-sl) is specified, -l becomes listenIP and -sl becomes targetIP
		// Otherwise, both listenIP and targetIP are -l (same IP)
		listenIP := lIP
		targetIP := lIP
		if slIP != "" {
			targetIP = slIP
		}
		pairs = append(pairs, forwardPair{listenIP: listenIP, targetIP: targetIP, targetPort: LPort})
	}
